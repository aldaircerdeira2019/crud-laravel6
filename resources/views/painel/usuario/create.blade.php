...
@section('conteudo')
	...
	<h1>Gestão de vendedores</h1>
		...
		<input type="text" name="nome" placeholder="nome do vendedo" value="{{$usuarios->nome ?? old('nome')}}"></div>
		<div class="form-group">
		<input type="password" name="senha" placeholder="senha"></div>
		<div class="form-group">
		<input type="email" name="email" placeholder="e-mail"value="{{$usuarios->email ?? old('email')}}"></div>
		<div class="form-group">
		<input type="text" name="cep" id="cep" placeholder="cep" value="{{$usuarios->cep ?? old('cep')}}" maxlength="9"
	               onblur="pesquisacep(this.value);"></div>
		<div class="form-group">
		<input type="text" name="rua" id='rua' placeholder="rua" value="{{$usuarios->rua ?? old('rua')}}"></div>
		<div class="form-group">
